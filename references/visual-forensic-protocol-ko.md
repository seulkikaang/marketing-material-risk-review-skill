...
- 손이 보이면 먼저 `left_hand`, `right_hand`를 각각 따로 crop한다고 가정하고 본다. 전체 인상으로 통과시키지 않는다.
- 엄지와 검지의 거리, 각도, 손등/손바닥 방향이 논란 상징과 유사한가?
- 검지가 무언가를 가리키는 포즈라도, 엄지·검지 사이의 빈 공간이 집게손처럼 읽히는가?
- 엄지 또는 검지가 짧고 둥글게 말려 작은 간격을 만드는가? 나머지 손가락이 접혀 그 간격이 더 강조되는가?
- 휴대폰, 카드, 쿠폰, 작은 상품, 동전, 소시지, 가격/할인 문구처럼 `작은 것`을 떠올리게 하는 소재와 결합했는가?
- 손동작이 제품 사용에 필요한가, 아니면 장식적으로 삽입됐는가?
- 손모양 주변에 특정 문구, 숫자, 오브젝트가 해석을 강화하는가?
- 영상이라면 해당 제스처가 몇 프레임만 등장하는가?

놓치기 쉬운 케이스:

- `pointing_but_pinching_like`: 검지는 포인터처럼 뻗었지만 엄지와의 간격/곡선 때문에 집게손처럼 보이는 손.
- `holding_phone_plus_second_hand`: 한 손은 휴대폰을 들고, 다른 손은 휴대폰을 가리키며 엄지·검지 간격이 강조되는 구성.
- `cartoon_simplification`: 일러스트라 손가락이 단순화되어 실제 의도보다 제스처가 더 명확하게 보이는 구성.

판정:

- 기능적 손동작이고 엄지-검지 빈 공간이 강조되지 않으면 `PASS` 또는 `LOW`.
- 포인팅/설명 제스처로도 읽히지만 집게손 유사성이 있으면 `REVIEW`가 아니라 최소 `REVISE`로 둔다.
- 작은 상품·가격·혜택·할인 문구와 결합해 집게손 해석이 강화되면 `HIGH_RISK`까지 올린다.
